...
class FLBridge:
    ...
    def call(
        ...
        *,
        ...
        try:
            ...
            if not slot.event.wait(wait_time):
                raise FLTimeout(
                    f"FL Studio did not respond to {command!r} within {wait_time:.1f}s. "
                    "FL Studio may have a modal dialog open (e.g. Save As, Plugin Scan, or an effect modal), "
                    "the audio engine may be frozen, or the 'FLStudioMCP' controller script is not enabled "
                    f"on MIDI port '{self._port_to_fl_pattern}'."
                )
    ...
    def send_raw_midi(self, data: bytes | list[int]) -> None:
        """Send raw MIDI bytes directly to FL Studio's input port."""
        self.check_alive()
        with self._lock:
            if self._out_port is not None:
    ...
    def send_midi_message(self, msg: Any) -> None:
        """Send a mido.Message directly to FL Studio's input port."""
        self.check_alive()
        with self._lock:
            if self._out_port is not None:
                self._out_port.send(msg)
...
class TCPBridge:
    ...
    def send_raw_midi(self, data: bytes | list[int]) -> None:
        """Send raw MIDI bytes via the daemon."""
        raw_list = list(data) if isinstance(data, (bytes, bytearray)) else list(data)
        try:
            self._rpc({"op": "send_raw_midi", "data": raw_list}, timeout=5.0)
        except OSError as e:
            raise self._daemon_unreachable(e)

    def send_midi_message(self, msg: Any) -> None:
        """Send a MIDI message via the daemon."""
        bytes_data = list(msg.bytes()) if hasattr(msg, "bytes") else list(msg)
        self.send_raw_midi(bytes_data)
