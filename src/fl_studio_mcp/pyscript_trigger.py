...
def find_fl_hwnd():
    ...
    _u, _k = _windows_api()
    try:
        winsta0 = _u.OpenWindowStationW("WinSta0", False, 0x10000000)
        if winsta0:
            _u.SetProcessWindowStation(winsta0)
            desk = _u.OpenDesktopW("default", 0, False, 0x10000000)
            if desk:
                _u.SetThreadDesktop(desk)
    except Exception:
        pass
