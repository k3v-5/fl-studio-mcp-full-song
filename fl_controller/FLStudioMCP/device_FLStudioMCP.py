...
def OnMidiMsg(event):
    ...
    # Backward compatibility with Franco Donati's FL-STUDIO-MCP transport trigger:
    # Note 76: Start recording
    # Note 77: Stop recording
    try:
        status = getattr(event, "status", 0)
        data1 = getattr(event, "data1", 0)
        data2 = getattr(event, "data2", 0)
...
def _h_channel_set_grid_bits(p):
    c = int(p["channel"])
    pat = p.get("pattern")
    if pat is not None:
        try:
            patterns.jumpToPattern(int(pat))
        except Exception:
            pass
    steps = p.get("steps", [])
    ...
    for s in steps:
        if isinstance(s, dict):
            ...
            v = val
        try:
            channels.setGridBit(c, step_idx, v)
        except Exception as e:
            return {"ok": False, "error": "setGridBit: %s" % e}
    return {"ok": True, "channel": c, "steps_count": len(steps)}
...
def _h_pattern_select(p):
    ...
    return {"ok": True, "pattern": patterns.patternNumber(), "name": patterns.getPatternName(pat)}


def _h_pattern_set_length(p):
    pat = int(p.get("pattern", patterns.patternNumber()))
    length = int(p["length"])
    try:
        patterns.setPatternLength(pat, length)
    except Exception as e:
        return {"ok": False, "error": "setPatternLength: %s" % e}
    return {"ok": True, "pattern": pat, "length": length}
...
_HANDLERS = {
    ...
    "channel_set_grid_bits": _h_channel_set_grid_bits,
